...
class MinimalCardDelegate: NSObject, OPPCardControllerDelegate {
    ...
    func submitPaymentTransaction() async -> Bool {
        // Just simulate validation success
        return true
    }

    // Completion-handler version
    func submitPaymentTransaction(completionHandler: @escaping (Bool) -> Void?) {
        completionHandler(true)
    }

    // Optional methods
    func cancelCheckout() {}
    func isCardScanAvailable() -> Bool { return false }
    func startCardScan() {}
    func checkoutSettings() -> OPPCheckoutSettings { OPPCheckoutSettings() }
    func token() -> OPPToken? { return nil }
